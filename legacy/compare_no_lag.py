...
def main():
    ...
    stop_loss = 0.10
    skip_lag = False # TESTING WITHOUT LAG
    
    # 1. Balanced (90 Lookback, Rank 30 Exit)
    balanced_params = {
        'momentum_lookback_days': 90, 'skip_latest_period': skip_lag, 'portfolio_size': 10,
        'drawdown_limit': stop_loss, 'exit_rank': 30, 'entry_rank': 10,
        'liquidate_on_bear_market': True, 'allocate_in_gold': True
    }
    
    # 2. Aggressive (60 Lookback, Rank 50 Exit)
    aggressive_params = {
        'momentum_lookback_days': 60, 'skip_latest_period': skip_lag, 'portfolio_size': 10,
        'drawdown_limit': stop_loss, 'exit_rank': 50, 'entry_rank': 10,
        'liquidate_on_bear_market': True, 'allocate_in_gold': True
    }

    configs = [("Balanced (No Lag)", balanced_params), ("Aggressive (No Lag)", aggressive_params)]
    results = []
    ...
    print("\n" + "="*60)
    print("WEEKLY REBALANCE (10 YEARS) - NO MOMENTUM LAG")
    print("="*60)
