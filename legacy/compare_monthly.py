...
def run_single_backtest(params, data_mgr, bench_prices, gold_prices, bt_start, bt_end, initial_capital_per_slot, liquid_yield, rebalance_freq):
    ...
    for date, momentum_row in monthly_momentum.iterrows():
        ...
        if not is_bull_market and liquidate_on_bear:
            ...
            target_portfolio = strategy.get_target_portfolio(momentum_row, current_holdings, current_prices, current_highs, market_bullish=is_bull_market)
...
def main():
    ...
    # 1. Balanced (90 Lookback, Rank 30 Exit, Monthly)
    balanced_params = {
        'momentum_lookback_days': 90, 'skip_latest_period': True, 'portfolio_size': 10,
        'drawdown_limit': 0.10, 'exit_rank': 30, 'entry_rank': 10,
        'liquidate_on_bear_market': True, 'allocate_in_gold': True
    }
    
    # 2. Aggressive (60 Lookback, Rank 50 Exit, Monthly)
    aggressive_params = {
        'momentum_lookback_days': 60, 'skip_latest_period': True, 'portfolio_size': 10,
        'drawdown_limit': 0.12, 'exit_rank': 50, 'entry_rank': 10,
        'liquidate_on_bear_market': True, 'allocate_in_gold': True
    }

    configs = [("Balanced (Monthly)", balanced_params), ("Aggressive (Monthly)", aggressive_params)]
    results = []
    
    for name, params in configs:
        print(f"Running {name}...")
        curve, trades = run_single_backtest(params, data_mgr, bench_prices, gold_prices, bt_start, bt_end, cap_cfg['allocation_per_slot'], cap_cfg.get('liquid_etf_yield', 0.065), 'BME')
    ...
    print("\n" + "="*60)
    print("MONTHLY REBALANCE COMPARISON (10 YEARS)")
    print("="*60)
